...
namespace VisiPlacement.Android
{
    public class AndroidTextMeasurer : TextMeasurer
    {
        ...
        {
            double fontMultiplier = 64; // Android has rounding error that we try to compensate for

            double measureFontSize = fontSize * fontMultiplier;

            TextView textView = new TextView(global::Android.App.Application.Context);
            textView.Typeface = this.getTypeface(fontName);
            textView.SetTextSize(ComplexUnitType.Px, (float)measureFontSize);
            ...
            return new Xamarin.Forms.Size(textView.MeasuredWidth / fontMultiplier, textView.MeasuredHeight / fontMultiplier);
        }
