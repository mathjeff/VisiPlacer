...
namespace VisiPlacement
{
    ...
    public class GridLayout : LayoutChoice_Set
    {
        ...
        public override SpecificLayout GetBestLayout(LayoutQuery query)
        {
            ...
            SemiFixed_GridLayout layout = this.GetBestLayout(query, new SemiFixed_GridLayout(this.wrappedChildren, this.rowHeights, this.columnWidths, this.bonusScore, setWidthBeforeHeight, query));
            ...
            while (currentCoordinate >= 0)
            {
                ...
                if (increaseScore)
                {
                    ...
                    if (newSublayout == null)
                    {
                        ...
                        if (semiFixedLayout.NextCoordinateAffectsWidth)
                            subQuery = query.New_MinWidth_LayoutQuery(maxWidth, maxHeight, minScore);
                        else
                            subQuery = query.New_MinHeight_LayoutQuery(maxWidth, maxHeight, minScore);
            ...
            foreach (int columnNumber in indices)
            {
                ...
                for (rowNumber = 0; rowNumber < this.rowHeights.NumProperties; rowNumber++)
                {
                    ...
                    if (subLayout != null)
                    {
                        ...
                        double queryHeight = layout.GetHeightCoordinate(rowNumber);
                        ...
                        if (runAllSubqueries)
                        {
                            ...
                            LayoutQuery query2 = sourceQuery.New_MinWidth_LayoutQuery(queryWidth, queryHeight, bestLayout.Score.Minus(eachAllowedScoreDecrease));
                        ...
                        if (currentWidth > maxRequiredWidth)
                        {
                            ...
                            if (maxRequiredWidth == queryWidth)
                            {
                                break;
                            }
                        }
            ...
            foreach (int rowNumber in indices)
            {
                ...
                for (columnNumber = 0; columnNumber < this.columnWidths.NumProperties; columnNumber++)
                {
                    ...
                    if (subLayout != null)
                    {
                        ...
                        {
                            // figure out how far the view can shrink while keeping the same score
                            LayoutQuery query2 = sourceQuery.New_MinHeight_LayoutQuery(queryWidth, queryHeight, bestLayout.Score.Minus(eachAllowedScoreDecrease));
                            query2.Debug = sourceQuery.Debug;
                        ...
                        if (currentHeight > maxRequiredHeight)
                        {
                            ...
                            if (maxRequiredHeight == queryHeight)
                            {
                                break;
                            }
                        }
        ...
        private SemiFixed_GridLayout ShrinkLayout(SemiFixed_GridLayout layout, bool enableDebugging)
        {
            ...
            for (rowNumber = layout.GetNumHeightProperties() - 1; rowNumber >= 0; rowNumber--)
            {
                ...
                for (columnNumber = layout.GetNumWidthProperties() - 1; columnNumber >= 0; columnNumber--)
                {
                    ...
                    if (currentLayout != null)
                    {
                        ...
                        SpecificLayout subLayout = layout.GetChildLayout(columnNumber, rowNumber);
            ...
            LayoutQuery sourceQuery = layout.SourceQuery;
    ...
    {
        private SemiFixed_GridLayout()
        {
            this.Initialize();
        }
        public SemiFixed_GridLayout(bool setWidthBeforeHeight, LayoutQuery sourceQuery)
        {
            this.Initialize();
        ...
        public SemiFixed_GridLayout(LayoutChoice_Set[,] elements, BoundProperty_List rowHeights, BoundProperty_List columnWidths, LayoutScore bonusScore, bool setWidthBeforeHeight, LayoutQuery sourceQuery)
        {
            this.Initialize();
            this.elements = elements;
            int numRows = elements.GetLength(1);
            int numColumns = elements.GetLength(0);
            this.sub_specificLayouts = new SublayoutResponse[this.elements.GetLength(0), this.elements.GetLength(1)];
        ...
        private SpecificLayout get_cached_specificLayout(int columnIndex, int rowIndex)
        {
            ...
            SublayoutResponse las = this.sub_specificLayouts[columnIndex, rowIndex];
        ...
        protected override bool isScoreAtLeast(LayoutQuery query)
        {
            ...
            for (rowNumber = 0; rowNumber < this.rowHeights.NumProperties; rowNumber++)
            {
                ...
                {
                    width = this.columnWidths.GetValue(columnNumber);
                    // Check whether we already know the score of this layout
                    SpecificLayout cachedLayout = this.get_cached_specificLayout(columnNumber, rowNumber);
                    if (cachedLayout != null)
                    {
                        // We already know the score of this layout, so we can use it
                        currentScore = currentScore.Plus(cachedLayout.Score);
                    }
                    else
                    {
            ...
            while (unscoredLayouts.Count > 0)
            {
                ...
                for (int i = 0; i < unscoredLayouts.Count; i++)
                {
                    ...
                    LayoutQuery subQuery = query.New_MaxScore_LayoutQuery(layoutAndSize.Size.Width, layoutAndSize.Size.Height, averageRequiredExtraScore);
        ...
        public override IEnumerable<SpecificLayout> GetParticipatingChildren()
        {
            ...
            for (int i = 0; i < this.columnWidths.NumProperties; i++)
            {
                ...
                {
                    SublayoutResponse las = this.sub_specificLayouts[i, j];
                    if (las != null)
                    {
                        SpecificLayout candidate = las.Layout;
                        if (candidate != null)
        ...
        SublayoutResponse[,] sub_specificLayouts;
    }
    ...
    class SublayoutResponse
    {
        public SublayoutResponse(SpecificLayout layout, Size size)
        {
            this.Layout = layout;
            this.QuerySize = size;
        }
        public SpecificLayout Layout;
